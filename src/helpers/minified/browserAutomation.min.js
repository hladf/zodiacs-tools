function tryClaim() {
  ...
  String(e?.innerText).toUpperCase().includes("CLAIM") &&
    (console.log("encontrado botao de CLAIM"), e?.click && e.click());
}
function tryStartRace() {
  ...
  String(e?.innerText).toUpperCase().includes("START RACE") &&
    (console.log("encontrado botao de START RACE"), e?.click && e.click());
}
function tryCheckRes() {
  const e = document.querySelector(
    "#root > section > section > div > div > div > div > div > div > div.mt-3 > div"
  );
  String(e?.innerText).toUpperCase().includes("CHECK RESULT") &&
    (console.log("encontrado botao de CHECK RESULT"), e?.click && e.click());
}
function runCarSelection(e) {
  ...
  for (let e = 0; e < o; e++) {
    ...
    if (
      (console.log(`racing count do carro ${e + 1} -> ${t}`),
      !n && String(t).includes("/10") && !String(t).includes("10/10"))
    ) {
...
function runAll(e = 60, o = 5) {
  ...
  setTimeout(() => {
    clearInterval(t), console.log("encerrando loop...");
  }, 1e3 * e);
}
runAll(360);
